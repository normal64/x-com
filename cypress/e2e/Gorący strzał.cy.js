describe("template spec", () => {
  it("passes", () => {
    ...
    const soldOutStatus = document.querySelector(".sc-1bb6kqq-11.eawDqC");
    if(soldOutStatus){
       //jesli wyprzedane
    ...
    .then((text) => {
      ...
      if (text === "Wyprzedano") {
        ...
        cy.get(
          ".sc-1h16fat-0.sc-153gokr-0.dMrHJf.sc-ikk4le-9.cMOIyV"
        ).click();
        return;
        }
    })
        ...
        cy.get(".sc-1bb6kqq-4.beOjbI").should(($el) => {
          const text = $el.text();
        ...
        cy.get(".sc-ntliq5-4.ciZKFP")
          ...
          .then((text) => {
            ...
            if (numberValue > 0) {
              cy.get("#hotShot").click();
              cy.get('[data-name="addToBasketButton"]')
                .first()
                .should("be.enabled");
            } else {
              //sprawdzamy sekundy
